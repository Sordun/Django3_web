...
class ShopTestCases(TestCase):
    ...
    def setUp(self):
        ...
        self.customer = Customer.objects.create(user=self.user, phone="987654321", address="Adres")
    ...
    def test_mock_homepage(self):
        ...
        with mock.path('mainapp.views.BaseView.get', return_value=mock_data) as mock_data_:
            factory = RequestFactory()
            request = factory.get('')
            request.user = self.user
